...
template<class T> using has_ne_impl = std::is_convertible<decltype( std::declval<T const&>() != std::declval<T const&>() ), bool>;
template<class T> using has_ne = mp11::mp_valid_and_true<has_ne_impl, T>;

} // namespace detail

template< class... T, class En = mp11::mp_if<mp11::mp_all<detail::has_ne<T>...>, void> >
constexpr bool operator!=( variant<T...> const & v, variant<T...> const & w )
{
